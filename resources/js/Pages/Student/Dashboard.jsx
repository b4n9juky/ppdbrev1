...
const noteStyleConfig = {
    draft: { bg: 'bg-amber-50 border-amber-200', text: 'text-amber-800', header: 'Catatan Perbaikan dari Operator', icon: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z' },
    rejected: { bg: 'bg-red-50 border-red-200', text: 'text-red-800', header: 'Alasan Penolakan Berkas', icon: 'M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z' },
    pending: { bg: 'bg-blue-50 border-blue-200', text: 'text-blue-800', header: 'Catatan Verifikasi', icon: 'M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
    accepted: { bg: 'bg-emerald-50 border-emerald-200', text: 'text-emerald-800', header: 'Catatan Kelulusan', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
    reserve: { bg: 'bg-amber-50 border-amber-200', text: 'text-amber-800', header: 'Catatan Cadangan', icon: 'M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
};
...
export default function Dashboard({ activeYear, registration, madrasah, documentTypes = [] }) {
    ...
    const activeNotificationNotes = registration 
        ? (registration.processing_status === 'selesai'
            ? {
                pending: 'verifikasi telah dilakukan data anda lolos verifikasi.',
              ...
              }[registration.status] || registration.verification_notes
                ...
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
                    {!registration ? (
                        ...
                        <>
                            ...
                            {activeNotificationNotes && (
                                <div className={`mb-8 overflow-hidden rounded-2xl border p-5 shadow-sm ${noteStyleConfig[registration.status]?.bg || 'bg-gray-50 border-gray-200'}`}>
                                    <div className="flex gap-3">
                                        <svg className={`h-5 w-5 shrink-0 mt-0.5 ${(noteStyleConfig[registration.status] || {}).text || 'text-gray-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={(noteStyleConfig[registration.status] || {}).icon || 'M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z'} />
                                        </svg>
                                        <div>
                                            <span className={`font-bold block text-sm ${(noteStyleConfig[registration.status] || {}).text || 'text-gray-900'}`}>
                                                {(noteStyleConfig[registration.status] || {}).header || 'Catatan Verifikasi'}
                                            </span>
                                            <p className={`mt-1 text-sm whitespace-pre-wrap ${((noteStyleConfig[registration.status] || {}).text || 'text-gray-700')}`}>
                                                {activeNotificationNotes}
                                            </p>
                                        </div>
                                    </div>
